...
export default function PriceChart({ livePrice, selectedAsset = "ETH/USDT" }: PriceChartProps) {
  ...
  useEffect(() => { setData(generateChartHistory(60)); }, [selectedAsset]);
    ...
    const interval = setInterval(() => {
      setData((prev) => {
        ...
        return [...prev.slice(1), {
          time: timeStr,
          binance: livePrice,
          coinbase: parseFloat(Math.max(3700, Math.min(4000, coinbaseDrift)).toFixed(2)),
  ...
  const formatPrice = useCallback((value: number) => `$${value.toFixed(0)}`, []);
      ...
      <div className="flex-1 min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
            ...
            <XAxis dataKey="time" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#6b7280" }} interval={9} />
            <YAxis domain={["auto", "auto"]} axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#6b7280" }} tickFormatter={formatPrice} width={50} />
            <Tooltip
